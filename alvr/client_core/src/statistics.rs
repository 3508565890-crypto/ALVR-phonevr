...
pub struct StatisticsManager {
    ...
    diagnostics: std::sync::Arc<FpsDiagnostics>,
...
impl StatisticsManager {
    pub fn new(
        ...
        Self {
            ...
            diagnostics: crate::statistics::get_diagnostics_arc(),
    ...
    pub fn report_video_packet_received(&mut self, target_timestamp: std::time::Duration) {
        ...
        self.diagnostics.received.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    ...
    pub fn report_frame_decoded(&mut self, target_timestamp: std::time::Duration) {
        ...
        self.diagnostics.decoded.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        self.maybe_log_diagnostics();
    }
    pub fn report_compositor_start(&mut self, target_timestamp: std::time::Duration) {
        ...
        self.diagnostics.compositor_start.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        self.maybe_log_diagnostics();
    }
    ...
    pub fn report_submit(&mut self, target_timestamp: std::time::Duration, vsync_queue: std::time::Duration) {
            ...
            .find(|frame| frame.client_stats.target_timestamp == target_timestamp)
        {
            ...
            if let Some(last_ts) = self.last_display_timestamp {
                let gap = target_timestamp.saturating_duration_since(last_ts);
                let gap_ms = gap.as_millis() as u64;
                if gap_ms == 0 {
                    self.diagnostics.timestamp_duplicate.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                } else if gap_ms > 50 {
                    self.diagnostics.timestamp_gap_over_50ms.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                }
                self.diagnostics.max_gap_ms.fetch_max(gap_ms, std::sync::atomic::Ordering::Relaxed);
            }
            self.last_display_timestamp = Some(target_timestamp);
        }
        self.diagnostics.submit.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        self.maybe_log_diagnostics();
    }
